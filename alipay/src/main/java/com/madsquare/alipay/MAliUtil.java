...
public class MAliUtil {
    private static final String TAG = MAliUtil.class.getSimpleName();
    ...
    public static String getOrderInfo(String productName, String productDetail, String price, Activity activity) {
        ...
        // 签约合作者身份ID
        String orderInfo = "partner=" + "\"" + MAliConfig.getPartnerID(activity) + "\"";

        // 签约卖家支付宝账号
        orderInfo += "&seller_id=" + "\"" + MAliConfig.getSeller(activity) + "\"";
        ...
        // 服务器异步通知页面路径
        orderInfo += "&notify_url=" + "\"" + MAliConfig.NOTIFY_URL + "\"";

        // 服务接口名称， 固定值
        orderInfo += "&service=\"" + MAliConfig.INTERFACE_NAME + "\"";

        // 支付类型， 固定值
        orderInfo += "&payment_type=\"" + MAliConfig.PAYMENT_TYPE + "\"";

        // 参数编码， 固定值
        orderInfo += "&_input_charset=\"" + MAliConfig.INPUT_CHARSET + "\"";
        ...
        orderInfo += "&it_b_pay=\"" + MAliConfig.TRANSACTION_TIMEOUT + "\"";

        // extern_token为经过快登授权获取到的alipay_open_id,带上此参数用户将使用授权的账户进行支付
//        orderInfo += "&extern_token=" + "\"" + MAliConfig.EXTERNAL_TOKEN + "\"";

        // 支付宝处理完请求后，当前页面跳转到商户指定页面的路径，可空
        orderInfo += "&return_url=\"" + MAliConfig.RETURN_URL + "\"";

        // 调用银行卡支付，需配置此参数，参与签名， 固定值 （需要签约《无线银行卡快捷支付》才能使用）
//        orderInfo += "&paymethod=\"" + MAliConfig.EXPRESS_GATEWAY + "\"";
        orderInfo += "&app_id=\"" + MAliConfig.getAppId(activity) + "\"";
